...
router.patch('/auth/profile', async (req: Request, res: Response) => {
  ...
  const {
    ...
    username, displayName, bio, birthday, timezone, city,
    discoverableByUsername, discoverableByEmail,
  } = req.body as {
    firstName?: string; lastName?: string; profileImageUrl?: string;
    username?: string; displayName?: string; bio?: string;
    birthday?: string; timezone?: string; city?: string;
    discoverableByUsername?: boolean; discoverableByEmail?: boolean;
  };

  // Validate username format if provided
  if (username !== undefined) {
    if (!/^[a-z0-9_]{3,24}$/.test(username)) {
      res.status(400).json({ error: 'Username must be 3-24 chars: letters, numbers, and underscores only' });
      return;
    }
    // Check uniqueness
    const existing = await db.query.usersTable.findFirst({ where: eq(usersTable.id, req.user.id) });
    if (existing && (existing as any).username !== username) {
      const taken = await db.select({ id: usersTable.id }).from(usersTable)
        .then((all: any[]) => all.find((u: any) => u.username === username && u.id !== req.user.id));
      if (taken) {
